...
class Admin extends User {
  ...
  async registerAdmin(data) {
    ...
    try {
      ...
      return {
        errors: {
          twilioError: "Error creating twilio subaccount - Contact Support",
        },
        data,
      };
    }
    ...
    const store = {
      ...
      storedata: {
        ...
        api: {
          twilio: {
            authToken: twilioAccount?.authToken || null,
            sid: twilioAccount?.sid || null,
            numSmsSent: 0,
            numSmsReceived: 0,
          },
        },
  ...
  async updateStoreTaxRate(storename, taxRate) {
    await storesCollection.updateOne(
      ...
      {
        $set: {
          "storeSettings.payments.taxRate": taxRate.replace(/^\D+/g, ""),
        },
      }
    );
  }
