...
class Ticket {
  ...
  async createNewTicket(formData, user) {
    ...
    if (!storeCustomers.hasOwnProperty(formData.phone)) {
      ...
      if (
        ...
        return [
          {
            firstnameError: "Firstname doesnt match account on file",
          },
          formData,
        ];
      } else if (
    ...
    await storesCollection.updateOne(
      ...
      },
      {
        $set: {
          ...
          [`storedata.tickets.${[ticketID]}.lastUpdated`]: latestUpdate,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.status`]:
            selection,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.lastUpdated`]:
            latestUpdate,
        },
      }
    );
    ...
    await storesCollection.updateOne(
      ...
      },
      {
        $set: {
          ...
          [`storedata.tickets.${[ticketID]}.lastUpdated`]: latestUpdate,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.issue`]:
            selection,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.lastUpdated`]:
            latestUpdate,
        },
  ...
  async updateTicketInfo(storename, newInfo, phone) {
    ...
    await storesCollection.updateOne(
      ...
      {
        $set: {
          [`storedata.tickets.${[ticketID]}.description`]: newDescription,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.description`]:
            newDescription,
          [`storedata.tickets.${[ticketID]}.subject`]: newSubject,
          [`storedata.customers.${[phone]}.tickets.${[ticketID]}.subject`]:
            newSubject,
        },
      }
    );
    ...
    await storesCollection.updateOne(
      { storename: storename },
      {
        $set: {
          ...
          [`storedata.tickets.${[ticketID]}.shipping.carrier`]: carrier,
          [`storedata.tickets.${[ticketID]}.shipping.tracking`]: trackingNumber,
        },
      }
    );
  }
  ...
  async receiveSms(smsData) {
    ...
    for (const ticket of ticketsForMsg) {
      ...
      await this.updateTicketStatus("Reply", ticket, fromNumber, storename);
  ...
  async trackShipment(ticketID, storename) {
    ...
    const { tracking, carrier } = store.storedata.tickets[ticketID].shipping;
