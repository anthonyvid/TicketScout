...
const showLoadingAnimation = async () => {
	...
	return new Promise(async (resolve, reject) => {
		await showSuccessMessage(
			"Congratulations! Time to get organized.",
			3000
		);
...
for (const input of inputs1) {
	input.addEventListener("input", () => {
		if (fullnameInput.value && emailInput.value && signUpCodeInput.value) {
...
nextBtn.addEventListener("click", async () => {
	...
	if (!signUpCodeInput.value) {
		helper.showInvalidColour(signUpCodeInput);
		return;
	}
...
registerBtn.addEventListener("click", async () => {
	...
	const data = await helper.postReq("/employee-register", {
		fullname: fullnameInput.value,
		email: emailInput.value,
		signUpCode: signUpCodeInput.value,
